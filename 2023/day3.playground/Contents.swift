...
var partNumbers: [[(Int, Int)?]] = input.map { $0.map { _ in nil } }
...
for (y, line) in input.enumerated() {
    ...
    for (x, char) in line.enumerated() {
        if isDigit(char) {
            ...
            approved = approved || hasSymbolNeighbor(x, y)
        } else {
            if candidate.count > 0, approved {
                let number = Int(candidate)!
                for i in 1...candidate.count {
                    partNumbers[y][x - i] = (id, number)
                }
                id += 1
            }
            approved = false
...
print("Start gear ratio")

for (y, line) in input.enumerated() {
    for (x, char) in line.enumerated() {
        if char == "*" {
            if let ratio = calculateGearRatio(x, y) {
                print("ratio \(ratio)")
                sum += ratio
            }
        }
...
func calculateGearRatio(_ x: Int, _ y: Int) -> Int? {
    var candidates: [Int] = []
    var ids: [Int] = []

    for offset in neighborOffsets {
        if let line = partNumbers[safe: y + offset.1] {
            if let partNumber = line[safe: x + offset.0] {
                if let partNumber = partNumber {
                    if !ids.contains(partNumber.0) {
                        ids.append(partNumber.0)
                        candidates.append(partNumber.1)
                        if ids.count > 2 {
                            return nil
                        }
                    }
    ...
    print("Gears: \(candidates[0]) \(candidates[1])")

    return candidates[0] * candidates[1]
}

func hasSymbolNeighbor(_ x: Int, _ y: Int) -> Bool {
    if isSymbol(x - 1, y) {
        return true
    }
    if isSymbol(x + 1, y) {
...
func isSymbol(_ x: Int, _ y: Int) -> Bool {
    if let line = input[safe: y] {
        if let character = line[safe: x] {
...
func isSymbol(_ input: Character) -> Bool {
    let result = !isDigit(input) && input != "."
//    print("\(input) -> \(result)")
    return result
}

func isDigit(_ input: Character) -> Bool {
    return Int(String(input)) != nil
}
