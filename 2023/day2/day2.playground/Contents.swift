...
for _line in input.split(separator: "\n") {
    let line = String(_line)

    let sep = line.split(separator: ":")
    let gameId = Int(String(sep[0]).dropFirst(5))!
    let gameData = sep[1]

    let reveals = gameData.split(separator: "; ")


    var max: [String: Int] = [:]

    revealer: for reveal in reveals {
        for item in reveal.split(separator: ", ") {
            let parts = item.split(separator: " ")
            let number = Int(parts[0]) ?? 0
